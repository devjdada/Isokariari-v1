...
export default function BlogCreate({ auth, categories }: PageProps<{ categories: Category[] }>) {
    const { data, setData, post, processing, errors, reset } = useForm<BlogFormData>({
        title: '',
        slug: '',
        content: '',
        description: '',
    ...
    const submit = (e: React.FormEvent) => {
        ...
        Object.entries(data).forEach(([key, value]) => {
            ...
            } else if (key === 'status' || key === 'is_published') {
    ...
    return (
        <AuthenticatedLayout user={auth.user} header={<h2 className="text-xl leading-tight font-semibold text-gray-800">Create Blog</h2>}>
                    ...
                    <div className="overflow-hidden bg-card p-6 shadow-sm sm:rounded-lg">
                        <form onSubmit={submit} className="mt-6 space-y-6">
                            ...
                            <div className="flex items-center space-x-2">
                                <Switch
                                    id="status"
                                    checked={data.status} // Convert string '1'/'0' to boolean
                                    onCheckedChange={(checked) => setData('status', checked)} // Convert boolean to string '1'/'0'
                                />
