...
export function toggleHighlight(editor: Editor | null, color: string, node?: Node | null, nodePos?: number | null): void {
    ...
    try {
        ...
        if (isEmptyNode(node)) {
            ...
            if (foundPos !== -1) {
                chain.setNodeSelection(foundPos).toggleMark('highlight', { color }).run();
            } else {
                chain.toggleMark('highlight', { color }).run();
            }
        } else {
 ...
 */
export function isColorHighlightButtonDisabled(editor: Editor | null, userDisabled: boolean = false): boolean {
 ...
 */
export function shouldShowColorHighlightButton(editor: Editor | null, hideWhenUnavailable: boolean, highlightInSchema: boolean): boolean {
